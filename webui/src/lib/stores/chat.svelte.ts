...
class ChatStore {
        ...
        private async streamChatCompletion(
                ...
                assistantMessageParam: DatabaseMessage,
                onComplete?: (content: string) => Promise<void>,
                onError?: (error: Error) => void,
                modelOverride?: string | null
        ): Promise<void> {
                let assistantMessage = assistantMessageParam;
                ...
                let streamedTextContent = '', // 正文内容（单独收集）
                        streamedReasoningContent = '', // 推理内容（单独收集）
                        streamedToolCallContent = '',
                        hasStreamedChunks = false,
                        resolvedModel: string | null = null,
                        modelPersisted = false;
                // Buffer for chunks that arrive during async message creation (wake notification race guard)
                let pendingChunks: string[] = [];
                let pendingReasoningChunks: string[] = [];
                let isPreparingWakeMessage = false;
                // The backend sends task_running=true at the START of EVERY AgentLoop,
                // not just for wake notifications. The first one is the normal start of the
                // user-initiated AgentLoop (sendMessage already created the assistant message).
                // Only task_running=true events AFTER the first done=true are genuine wake
                // notifications from async tasks completing.
                let isFirstTaskRunning = true;
                ...
                const buildCombinedContent = () => {
                        if (!streamedReasoningContent) return streamedTextContent;
                        return `${REASONING_TAGS.START}${streamedReasoningContent}${REASONING_TAGS.END}${streamedTextContent}`;
                };
                const updateStreamingContent = () => {
                        const combinedContent = buildCombinedContent();
                        this.setChatStreaming(assistantMessage.convId, combinedContent, assistantMessage.id);
                        const idx = conversationsStore.findMessageIndex(assistantMessage.id);
                        conversationsStore.updateMessageAtIndex(idx, { content: combinedContent });
                };
                const appendContentChunk = (chunk: string) => {
                        // If wake message is being created (async), buffer chunks to prevent
                        // writing to the old assistant message
                        if (isPreparingWakeMessage) {
                ...
                const appendReasoningChunk = (chunk: string) => {
                        if (isPreparingWakeMessage) {
                                pendingReasoningChunks.push(chunk);
                                return;
                        }
                ...
                const streamCallbacks: ChatStreamCallbacks = {
                        ...
                        onTaskRunning: async (running: boolean) => {
                                if (running) {
                                        ...
                                        // Silently consume it — only subsequent events after done=true
                                        ...
                                        // Wake notification: async task completed, new AgentLoop starting.
                                        // Set guard flag BEFORE async work to buffer any chunks that
                                        // arrive during the createAssistantMessage await (IndexedDB write).
                                        isPreparingWakeMessage = true;
                                        pendingChunks = [];
                                        ...
                                        // Create new assistant message (child of previous completed message)
                                        const newMessage = await this.createAssistantMessage(assistantMessage.id);
                                        conversationsStore.addMessageToActive(newMessage);
                                        assistantMessage = newMessage;

                                        // Now safe to accept chunks — reset streaming accumulators
                                        streamedTextContent = '';
                                        ...
                                        isPreparingWakeMessage = false;

                                        // Replay any chunks that were buffered during message creation
                                        for (const chunk of pendingChunks) {
                                                streamedTextContent += chunk;
                                                hasStreamedChunks = true;
                                        }
                                        for (const chunk of pendingReasoningChunks) {
                                                streamedReasoningContent += chunk;
                                                hasStreamedChunks = true;
                                        }
                                        pendingChunks = [];
                                        ...
                                        // Update UI with buffered content (if any) or empty state
                                        this.setActiveProcessingConversation(assistantMessage.convId);
