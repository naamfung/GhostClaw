...
type MCPClient struct {
    ...
    mu         sync.RWMutex
    // 工具定義緩存：避免每次 API 請求重新構建 tool definitions slice
    cachedToolDefs     []map[string]interface{}
    cachedToolDefsDirty bool // 標記緩存是否需要刷新
...
// 使用緩存機制：首次調用或工具更新後構建 slice，後續調用直接返回緩存。
func (c *MCPClient) GetToolDefinitions() []map[string]interface{} {
    c.mu.RLock()
    if !c.cachedToolDefsDirty && c.cachedToolDefs != nil {
        defs := c.cachedToolDefs
        c.mu.RUnlock()
        return defs
    }
    c.mu.RUnlock()
    ...
    // 雙重檢查（可能其他 goroutine 已重建）
    if !c.cachedToolDefsDirty && c.cachedToolDefs != nil {
        return c.cachedToolDefs
    }
    ...
    c.cachedToolDefs = definitions
    c.cachedToolDefsDirty = false
    return definitions
}
