...
export const AGENTIC_REGEX = {
        // Matches completed tool calls (with END marker, optionally captures TOOL_STATUS)
        COMPLETED_TOOL_CALL:
                /<<<AGENTIC_TOOL_CALL_START>>>\n<<<TOOL_NAME:(.+?)>>>\n<<<TOOL_ARGS_START>>>([\s\S]*?)<<<TOOL_ARGS_END>>>([\s\S]*?)(?:<<<TOOL_STATUS:(\w+)>>>\s*)?<<<AGENTIC_TOOL_CALL_END>>>/g,
        ...
        // Matches any orphaned agentic tag markers that leaked into text
        // (should never be visible to users)
        ORPHANED_AGENTIC_MARKERS: /<<<(?:AGENTIC_TOOL_CALL_START|AGENTIC_TOOL_CALL_END|TOOL_NAME:[^>]*|TOOL_ARGS_START|TOOL_ARGS_END|TOOL_STATUS:\w+|reasoning_content_start|reasoning_content_end)>>>/g
} as const;
