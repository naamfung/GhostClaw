...
func NewWebhookChannel(config *WebhookConfig) (*WebhookChannel, error) {
	...
	return &WebhookChannel{
		...
		// 持久化到 <dataDir>/webhook_results，遵守 DataDir 规则
		// （旧实现使用相对路径 "memory/webhook_results"，会落到 cwd/memory/webhook_results，
		// 既不在 data 目录下，又与 dataDir/memory 数据库目录混淆）
		resultDir: filepath.Join(globalDataDir, "webhook_results"),
	}, nil
}
...
func (wc *WebhookChannel) Start(messageHandler func(chatID, senderID, content string, metadata map[string]interface{})) error {
	...
	// 初始化結果持久化目錄並加載歷史結果
	if wc.resultDir != "" {
		if err := os.MkdirAll(wc.resultDir, 0755); err != nil {
			log.Printf("[Webhook] Warning: failed to create result directory %s: %v", wc.resultDir, err)
			wc.resultDir = "" // 回退到純內存模式
		} else {
			wc.loadAllPersistedResults()
		}
	}
...
func (wc *WebhookChannel) handleResultQuery(w http.ResponseWriter, r *http.Request) {
	...
	// 內存中找不到時，嘗試從文件加載（重啟後場景）
	if !exists {
		if fileResult, ok := wc.loadResultFromFile(taskID); ok {
			result = fileResult
			exists = true
			// 回填到內存緩存
			wc.resultsMu.Lock()
...
func (wc *WebhookChannel) handleWebhook(w http.ResponseWriter, r *http.Request) {
	...
	if HandleSlashCommandWithDefaults(req.Message,
		...
		func() {
			// /exit: 退出程序
			log.Println("[Webhook] Received /exit, exiting program...")
...
func (wc *WebhookChannel) processWebhookMessage(session *GlobalSession, taskID, channelID, senderID, input string, metadata map[string]interface{}, callbackURL string) {
	...
	// 获取当前模型配置（優先通過 ConfigManager 線程安全讀取）
	...
	// 执行 AgentLoop
	newHistory, err := AgentLoop(taskCtx, ch, history, effectiveAPIType, effectiveBaseURL, effectiveAPIKey, effectiveModelID, effectiveTemperature, effectiveMaxTokens, effectiveStream, effectiveThinking)
...
func (wc *WebhookChannel) storeResult(taskID string, result *WebhookResult) {
	...
	// 可選：持久化到文件，重啟後可恢復
	if wc.resultDir != "" {
		go wc.persistResultToFile(taskID, result)
	}
...
// persistResultToFile 將任務結果持久化到文件
func (wc *WebhookChannel) persistResultToFile(taskID string, result *WebhookResult) {
	if wc.resultDir == "" {
		return
	}
	filePath := filepath.Join(wc.resultDir, taskID+".json")
	data, err := json.Marshal(result)
	if err != nil {
		log.Printf("[Webhook] Failed to marshal result for persistence: %v", err)
		return
	}
	if err := os.WriteFile(filePath, data, 0644); err != nil {
		log.Printf("[Webhook] Failed to persist result to %s: %v", filePath, err)
	}
}

// loadResultFromFile 從文件加載任務結果
func (wc *WebhookChannel) loadResultFromFile(taskID string) (*WebhookResult, bool) {
	if wc.resultDir == "" {
		return nil, false
	}
	filePath := filepath.Join(wc.resultDir, taskID+".json")
	...
	if err := json.Unmarshal(data, &result); err != nil {
		log.Printf("[Webhook] Failed to unmarshal persisted result: %v", err)
		return nil, false
	}
	return &result, true
}

// loadAllPersistedResults 啓動時從文件加載所有持久化的結果
func (wc *WebhookChannel) loadAllPersistedResults() {
	if wc.resultDir == "" {
		return
	}
	entries, err := os.ReadDir(wc.resultDir)
	if err != nil {
		log.Printf("[Webhook] Failed to read result directory %s: %v", wc.resultDir, err)
		return
	}
	count := 0
	for _, entry := range entries {
		...
		if result, ok := wc.loadResultFromFile(taskID); ok {
			wc.resultsMu.Lock()
			wc.results[taskID] = result
			wc.resultsMu.Unlock()
			count++
	...
	if count > 0 {
		log.Printf("[Webhook] Loaded %d persisted task results from %s", count, wc.resultDir)
	}
}
