...
- **自我进化系统**：实现了完整的自我进化架构，包括数据收集、分析、策略优化、记忆重构和评分系统
- **循环检测配置外放**：循环检测警告消息可外部配置，支持自我进化系统优化，同时内置事件收集系统
- **系统信息注入**：可选择将内存、CPU、GPU 等系统信息注入模型提示词，辅助决策
...
| **IRC** | girc | 可选 | IRC 协议原生支持 |
| **Webhook** | net/http | 可选 | 通用 Webhook 接入 |
| **XMPP** | mellium/xmpp | 可选 | XMPP/Jabber 协议支持 |
...
GhostClaw 实现了完整的自我进化架构，通过数据收集、分析、优化和记忆管理，实现系统的持续改进。

### 核心组件

| 组件 | 职责 | 核心文件 |
|------|------|----------|
| **反馈收集器** | 智能收集用户反馈 | `feedback_collector.go` |
| **轨迹管理器** | 记录完整对话轨迹 | `trajectory.go` |
| **分析引擎** | 生成综合分析报告 | `insights.go` |
| **策略优化器** | 基于数据自动优化 | `strategy_optimizer.go` |
| **记忆重构管理器** | 主动管理和优化记忆 | `memory_refactor.go` |
| **记忆评分器** | 自动化记忆质量评估 | `memory_scorer.go` |

### 工作流程

1. **数据收集**：智能收集用户反馈和对话轨迹
2. **数据分析**：生成综合分析报告，识别改进机会
3. **策略优化**：基于分析结果自动调整系统策略
4. **记忆重构**：主动管理和优化记忆，清理过期和低质量记忆
5. **记忆评分**：自动化评估记忆质量，支持智能记忆管理
6. **持续改进**：形成完整的自我进化闭环

### 关键特性

- **智能反馈收集**：模型主动追问，用户无感知
- **完整轨迹记录**：支持成功/失败轨迹分类存储
- **综合分析报告**：多维度数据分析和智能建议
- **自动策略优化**：基于数据的智能决策
- **智能记忆管理**：清理、更新、合并、添加记忆
- **自动化记忆评分**：多维度的记忆质量评估

> 📖 **详细说明**：请参阅 [VERSION.md](./VERSION.md) 了解自我进化系统的版本演进。
...
├── xmpp_channel_simple.go # XMPP 通道（可选编译）
