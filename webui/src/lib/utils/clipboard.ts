...
export async function copyToClipboard(
        ...
        try {
                ...
                // 檢測是否在 focus trap 內（如 AlertDialog）。
                // 如果是，textarea 必須附加到 trap 容器內部才能獲得焦點，
                // 否則 focus trap 會把焦點搶回去，導致 select() 無效、複製失敗。
                const activeEl = document.activeElement;
                let container: HTMLElement = document.body;
                if (activeEl instanceof HTMLElement) {
                        const trap = activeEl.closest('[data-slot="alert-dialog-content"]') ||
