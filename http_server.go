...
func NewHTTPServer(addr string) *HTTPServer {
	// 上传目录统一放到 <dataDir>/uploads，遵守 DataDir 规则
	// （globalDataDir 在 main() 中已先于 NewHTTPServer 设置完成）
	uploadDir := filepath.Join(globalDataDir, "uploads")
	if err := os.MkdirAll(uploadDir, 0755); err != nil {
		log.Printf("Warning: failed to create uploads directory: %v", err)
	}
	globalUploadDir = uploadDir
