...
export class ModelsService {
        ...
        /**
         * Fetch router models with full status metadata.
         * In GhostClaw, this maps to /v1/models which returns OpenAI-compatible format with status.
         *
         * @returns Router model data entries with status info
         */
        static async listRouter(): Promise<{ data: ApiModelDataEntry[] }> {
                return apiFetch<{ data: ApiModelDataEntry[] }>('/v1/models');
        }

        /**
         * Load a model (ROUTER mode).
         * GhostClaw does not support dynamic model loading/unloading like llama-server.
         * This is a no-op that returns immediately to maintain interface compatibility.
         *
         * @param _modelId - Model identifier (ignored in GhostClaw)
         */
        static async load(_modelId: string): Promise<void> {
                // GhostClaw manages models statically via config, not dynamic loading
                console.info('GhostClaw does not support dynamic model loading. Models are managed via /api/models configuration.');
        }

        /**
         * Unload a model (ROUTER mode).
         * GhostClaw does not support dynamic model unloading like llama-server.
         * This is a no-op that returns immediately to maintain interface compatibility.
         *
         * @param _modelId - Model identifier (ignored in GhostClaw)
         */
        static async unload(_modelId: string): Promise<void> {
                // GhostClaw manages models statically via config, not dynamic unloading
                console.info('GhostClaw does not support dynamic model unloading. Models are managed via /api/models configuration.');
        }
        ...
        /**
         * Check if a GhostClaw model config is the main model.
         *
         * @param model - Model configuration from GhostClaw API
         * @param mainModelName - The name of the main model
         * @returns True if this is the main model
         */
        static isMainModel(model: ModelConfig, mainModelName: string): boolean {
                return model.Name === mainModelName;
        }
