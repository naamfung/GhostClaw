...
func (tc *TelegramChannel) splitMessage(text string, maxLen int) []string {
        ...
        for _, line := range lines {
                ...
                // If a single line exceeds maxLen, split it at rune boundaries
                runes := []rune(line)
                if len(runes) > maxLen {
                        for i := 0; i < len(runes); i += maxLen {
                                ...
                                if current.Len()+len(chunk)+1 > maxLen && current.Len() > 0 {
                                        messages = append(messages, current.String())
                                        current.Reset()
                                }
...
// Uses paired tag matching to avoid broken nesting.
func markdownToTelegramHTML(text string) string {
        // Process code blocks first - replace with placeholder to protect content
        ...
        for {
                ...
                codeBlocks = append(codeBlocks, text[start+3:end-3])
                text = text[:start] + fmt.Sprintf("\x00CODE%d\x00", len(codeBlocks)-1) + text[end+3:]
        }
        ...
        for {
                ...
                inlineCodes = append(inlineCodes, text[start+1:end-1])
                text = text[:start] + fmt.Sprintf("\x00INLINE%d\x00", len(inlineCodes)-1) + text[end+1:]
        }

        // Process bold with **paired**
        text = replacePairedMarkdown(text, "**", "<b>", "</b>")
        // Process bold with __paired__
        text = replacePairedMarkdown(text, "__", "<b>", "</b>")
        // Process italic with *paired*
        text = replacePairedMarkdown(text, "*", "<i>", "</i>")
        // Process italic with _paired_
        text = replacePairedMarkdown(text, "_", "<i>", "</i>")

        // Restore inline code
        for i, code := range inlineCodes {
                text = strings.ReplaceAll(text, fmt.Sprintf("\x00INLINE%d\x00", i), fmt.Sprintf("<code>%s</code>", code))
        }
        // Restore code blocks
        for i, code := range codeBlocks {
                text = strings.ReplaceAll(text, fmt.Sprintf("\x00CODE%d\x00", i), fmt.Sprintf("<pre>%s</pre>", code))
        }

        return text
}

// replacePairedMarkdown replaces paired markdown delimiters with HTML open/close tags.
// Only replaces the first occurrence of a pair; unmatched delimiters are left as-is.
func replacePairedMarkdown(text, marker, openTag, closeTag string) string {
        ...
        for {
                ...
                result.WriteString(text[first+len(marker):second])
                result.WriteString(closeTag)
                text = text[second+len(marker):]
        }
