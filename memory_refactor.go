...
func (rm *MemoryRefactorManager) performRefactoring(memories []MemoryEntry) []RefactorAction {
        ...
        if rm.memory == nil {
                log.Println("[MemoryRefactorManager] Memory backend is nil, skipping refactoring")
                return actions
        }

        // 1. 清理过期记忆 (30天以上)
        for _, memory := range memories {
                if time.Since(memory.CreatedAt).Hours() > 30*24 {
                        err := rm.memory.DeleteEntry(memory.Category, memory.Key)
                        success := err == nil
                        if err != nil {
                                log.Printf("[MemoryRefactorManager] Failed to delete expired memory %s/%s: %v", memory.Category, memory.Key, err)
                        }
        ...
        // 2. 更新低质量记忆 (score < 0.3)：刷新 access_cnt 和 updated_at，并标记已重构
        for _, memory := range memories {
                if memory.Score < 0.3 {
                        // Append a "refactored" tag to indicate the entry was reviewed
                        updatedTags := make([]string, len(memory.Tags))
                        copy(updatedTags, memory.Tags)
                        updatedTags = append(updatedTags, "refactored")

                        err := rm.memory.UpdateEntry(memory.Category, memory.Key, memory.Value, updatedTags)
                        success := err == nil
                        if err != nil {
                                log.Printf("[MemoryRefactorManager] Failed to update low-quality memory %s/%s: %v", memory.Category, memory.Key, err)
                        }
                        actions = append(actions, RefactorAction{
                                Type:        "update",
                                Category:    string(memory.Category),
