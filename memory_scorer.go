...
// scorerGetDB returns a safe DB reference, checking both nil and recovery state.
func scorerGetDB() *gorm.DB {
...
func (ms *MemoryScorer) UpdateMemoryScore(memoryID string, relevanceScore float64, feedbackScore float64) error {
        ...
        // 从数据库中获取记忆
        var memory Memories
        result := db.First(&memory, "id = ?", memoryID)
        if result.Error != nil {
                // 尝试从 Experiences 表中查找
                var experience Experiences
                result = db.First(&experience, "id = ?", memoryID)
                if result.Error != nil {
                ...
                // 更新数据库
                db.Model(&experience).Update("score", newScore)
        ...
        // 更新数据库
        db.Model(&memory).Update("score", newScore)
