...
记忆评分系统通过多维度评估记忆质量，支持智能记忆管理和自我进化。

#### 评分维度

| 维度 | 权重 | 说明 |
|------|------|------|
| 访问频率 | 30% | 基于访问次数，10次访问达到满分 |
| 新鲜度 | 20% | 基于最后访问时间，每天 1% 的评分衰减 |
| 相关性 | 30% | 基于记忆与当前上下文的相关性 |
| 反馈 | 20% | 基于用户反馈 |

#### 评分算法

- **加权综合评分**：访问(30%) + 新鲜度(20%) + 相关性(30%) + 反馈(20%)
- **评分范围**：0-1
- **自动衰减**：每天 1% 的评分衰减
- **评分归一化**：确保评分在合理范围内
...
- **与记忆重构系统集成**：基于评分进行记忆管理，清理低评分记忆
- **与自我进化系统集成**：评分数据作为优化决策的重要依据
- **自动化**：实时评分更新和批量评分更新

#### 核心文件

- `memory_scorer.go`：记忆评分器的核心实现
- `memory_refactor.go`：基于评分的记忆重构逻辑
