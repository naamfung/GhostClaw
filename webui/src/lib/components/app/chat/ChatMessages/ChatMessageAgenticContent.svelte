...
{#snippet renderSection(section: (typeof sectionsParsed)[number], index: number)}
        ...
        {:else if section.type === AgenticSectionType.TOOL_CALL || section.type === AgenticSectionType.TOOL_CALL_PENDING}
                ...
                {@const hasError = !isPending && section.hasError === true}
                {@const toolIcon = isPending ? Loader2 : (hasError ? AlertTriangle : Wrench)}
                {@const toolIconClass = isPending ? 'h-4 w-4 animate-spin' : (hasError ? 'h-4 w-4 text-yellow-500' : 'h-4 w-4')}
                ...
                >
                        ...
                        <div class="pt-3">
                                ...
                                {#if section.toolResult}
                                        <div class="overflow-y-auto rounded-lg border {hasError ? 'border-yellow-500/40 bg-yellow-500/5' : 'border-border bg-muted'} p-4" style="overflow-x: hidden;">
