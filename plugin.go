...
// validatePluginPath checks that a path is within the allowed plugin working directory
func validatePluginPath(path string) error {
        ...
        // Basic protection: block obvious system paths
        blockedPrefixes := []string{"/etc", "/proc", "/sys", "/dev", "/boot", "/usr", "/bin", "/sbin", "/var"}
        for _, prefix := range blockedPrefixes {
                if realPath == prefix || strings.HasPrefix(realPath, prefix+"/") {
                        return fmt.Errorf("access denied: system path not allowed (%s)", path)
                }
        }
        return nil
}
...
func (pm *PluginManager) registerAPIs(L *lua.LState, pluginName string) {
        ...
        L.SetField(ghostclaw, "upload_multipart", L.NewFunction(func(L *lua.LState) int {
                ...
                // SSRF protection
                if ssrfResult := CheckURLSSRF(url); !ssrfResult.Safe {
                        L.Push(lua.LBool(false))
                        L.Push(lua.LString(fmt.Sprintf("SSRF blocked: %s", ssrfResult.Reason)))
                        return 2
                }
