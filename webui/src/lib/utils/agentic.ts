...
export function parseAgenticContent(rawContent: string): AgenticSection[] {
        ...
        // Safety net: strip any orphaned agentic markers from TEXT sections.
        // Models fine-tuned on agentic data may occasionally leak markers into
        // their text output; these must never be visible to the user.
        return sections.map(section => {
                if (section.type === AgenticSectionType.TEXT && section.content) {
                        return {
                                ...section,
                                content: section.content.replace(AGENTIC_REGEX.ORPHANED_AGENTIC_MARKERS, '').trim()
                        };
                }
                return section;
        }).filter(section => {
                // Remove empty TEXT sections left after stripping markers
                if (section.type === AgenticSectionType.TEXT && !section.content) {
                        return false;
                }
                return true;
