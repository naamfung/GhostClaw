                ...
                for {
                        ...
                        case <-ticker.C:
                                if dc.ws != nil {
                                        ...
                                        ws := dc.ws
                                        dc.wsMu.Unlock()
                                        dc.heartbeatMu.RLock()
                                        seq := dc.seq
                                        dc.heartbeatMu.RUnlock()
                                        heartbeat := map[string]interface{}{
                                                "op": 1,
                                                "d":  seq,
                                        }
                                        ws.WriteJSON(heartbeat)
                                }
                        }
                }
        }()
...
func (dc *DiscordChannel) handleDispatch(eventType string, data json.RawMessage) {
        ...
        case "MESSAGE_CREATE":
                ...
                if HandleSlashCommandWithDefaults(msg.Content,
                        ...
                        func() {
                                // /exit: 退出程序
                                log.Println("[Discord] Received /exit, exiting program...")
...
func (dc *DiscordChannel) sendMessage(channelID, content string) error {
        ...
        for attempt := 0; attempt < maxRetries; attempt++ {
                ...
                if resp.StatusCode == 429 {
                        // Rate limited — explicitly close body before continue
        ...
        return fmt.Errorf("discord API: max retries (%d) exceeded for rate limiting", maxRetries)
}
