...
import { ModelsService, PropsService, type ModelConfig } from '$lib/services';
...
class ModelsStore {
        ...
        async fetch(force = false): Promise<void> {
                ...
                try {
                        ...
                        // GhostClaw 后端返回 { Models: [...], MainModel: "..." } 格式
                        const modelList = response.Models || [];
                        const mainModelName = response.MainModel || '';

                        const models: ModelOption[] = modelList.map((item: ModelConfig) => {
                                const modelId = item.Model || item.Name;
                                const displayName = this.toDisplayName(item.Name || modelId);
                                const isMain = item.IsDefault || false;
                                ...
                                return {
                                        ...
                                        details: undefined,
                                        meta: {
                                                ...((isMain ? { main: true } : {}) as Record<string, unknown>),
                                                apiType: item.APIType
                                        },
                                        parsedId: ModelsService.parseModelId(modelId),
                                        aliases: [] as string[],
                                        tags: [item.APIType].filter(Boolean) as string[]
...
// Re-export ModelConfig type for convenience
export type { ModelConfig } from '$lib/services';
