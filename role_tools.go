...
func GetHelpText() string {
        ...
        sb.WriteString("│\n")
        sb.WriteString("│  MemorySave        保存记忆（键值对 + 分类 + 标签）\n")
        sb.WriteString("│  MemoryRecall      检索记忆（按关键词搜索）\n")
        sb.WriteString("│  MemoryForget      删除指定记忆\n")
        sb.WriteString("│  MemoryList        列出所有记忆\n")
        sb.WriteString("│  ConsolidateMemory 生成记忆摘要\n")
        sb.WriteString("│\n")
        ...
        sb.WriteString("│\n")
        sb.WriteString("│  CronAdd           添加定时任务（名称、cron表达式、消息）\n")
        sb.WriteString("│  CronRemove        删除定时任务\n")
        sb.WriteString("│  CronList          列出所有定时任务\n")
        sb.WriteString("│  CronStatus        查询任务执行状态\n")
        sb.WriteString("│\n")
        sb.WriteString("│  示例（让 AI 执行）：\n")
        sb.WriteString("│    \"每天早上9点提醒我开会\" -> AI 调用 CronAdd\n")
        sb.WriteString("│    \"列出我的定时任务\"   -> AI 调用 CronList\n")
        sb.WriteString("└───────────────────────────────────────────────────────────────\n\n")
